...
let inflightConfig: Promise<AdminConfig> | null = null;

async function loadConfigFromStore(): Promise<AdminConfig> {
  // 读 db
...
export async function getConfig(): Promise<AdminConfig> {
  ...
  // Share one in-flight DB read. Do not keep a durable memory TTL: Docker
  // module state can outlive admin config writes.
  if (inflightConfig) return inflightConfig;
  const request = loadConfigFromStore().finally(() => {
    if (inflightConfig === request) inflightConfig = null;
  });
