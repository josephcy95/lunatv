...
export function createRedisClient(
  ...
  if (!client) {
    ...
    // Avoid opening sockets (or retry timers) merely by importing a storage
    // module in Jest. Integration tests can still call `client.connect()`
    // explicitly when a real Redis/Kvrocks service is available.
...
export abstract class BaseRedisStorage implements IStorage {
  ...
  async deleteUser(userName: string): Promise<void> {
    ...
    await this.withRetry(() => this.client.del(`u:${userName}:watch-status`));
    await this.withRetry(() => this.client.del(`u:${userName}:trakt-tokens`));
    await this.withRetry(() => this.client.del(`u:${userName}:emby-config`));
  }
